	...
	for (var i = 0; i < boardLength; i++) {
		for (var j = 0; j < boardLength; j++) {
			...
			if (board[i][j] == 2) { //pacman
				...
				{ //up
					context.beginPath();
					context.arc(center.x/2, center.y/2, half_square, 1.70 * Math.PI , 1.30 * Math.PI); // half circle
					context.lineTo(center.x/2, center.y/2);
					context.fillStyle = pac_color; //color
					context.fill();
					context.beginPath();
					context.arc(center.x - 12/2, center.y/2 , 5/2,2 * Math.PI, 0); // circle
					context.fillStyle = "black"; //color	
					context.fill();
				}
				else if (keyboardDiraction == 2){ //down
					context.beginPath();
					context.arc(center.x/2, center.y/2, square, 0.65 * Math.PI , 0.35 * Math.PI); // half circle
