...
function Start() {
	...
	// var food_remain_list = {"food_5_points_remain": food_5_points_remain, "food_15_points_remain": food_15_points_remain, "food_25_points_remain": food_5_points_remain};
	var pacman_remain = 1;
	start_time = new Date();
	for (var i = 0; i < 10; i++) {
		board[i] = new Array();
...
function UpdatePosition() {
	...
	time_elapsed = (currentTime - start_time);
	if(time_elapsed == document.getElementById("timer").value && score >= 100){
		// if(score < 100){
	...
	if (score < 100 && time_elapsed == document.getElementById("timer").value) {
		window.clearInterval(interval);
