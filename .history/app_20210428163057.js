...
function Start() {
	...
	for (var i = 0; i < boardLength; i++) {
		...
		for (var j = 0; j < boardLength; j++) {
			if (searchForArray(walls, [i,j]) != -1) {
				...
				if (randomNum <= (0.0001 * food_remain) / cnt) {					if("food_5_points_remain" in food_remain_list && food_remain_list["food_5_points_remain"] == 0){
	...
	strawInterval = setInterval(updateStrawPosition, 1000);
...
function getRandomDirS(straw){
	...
	var move = [];
		if(ghostRow-1 >= 0 && validSnextMove(ghostCol, ghostRow-1, 'up' , straw.prevDirection)){
			move.push('up');
		}
	else if(ghostRow+1 < boardLength && validSnextMove(ghostCol, ghostRow+1, 'down' , straw.prevDirection)){ 
			move.push("down")
		}
		else if(ghostCol-1 >= 0 && validSnextMove(ghostCol-1, ghostRow, 'left' , straw.prevDirection)){ 
			move.push("left");
		}
		else if(ghostCol+1 < boardLength && validSnextMove(ghostCol+1, ghostRow, 'right' , straw.prevDirection)){ 
			move.push("right");
		}
...
// function getMovesForDirection(wantedDir, listOtherDirs){
...
function validGhostNextMove(col, row){
	if(!notValidMove.includes(board[col][row])){
		return true;
	}
	return false;
}
		...
		for (var j = 0; j < boardLength; j++) {
			...
			if (board[i][j] == 2) { //pacman
				...
				{ //up
					...
					context.fill();
					context.beginPath();
					context.arc(center.x - 12, center.y , 5/2,2 * Math.PI, 0); // circle
				...
				{ //left
					...
					context.arc(center.x - 5, center.y - 11, 5/2, 0,2 * Math.PI); // circle
					context.fillStyle = "black"; //color
					context.fill();
				} 
				...
				//color5
				context.beginPath();
				context.arc(center.x, center.y, 6/2, 0, 2 * Math.PI); // circle
				...
				//color15
				context.beginPath();
				context.arc(center.x, center.y, 9/2, 0, 2 * Math.PI); // circle
				...
				context.beginPath();
				context.arc(center.x, center.y, 12/2, 0, 2 * Math.PI); // circle
			...
			// } else if (board[i][j] == 6) {
			// 	context.drawImage(listOfGhost[0].img , center.x-half_square, center.y-half_square ,square,square);
				
			// } else if (board[i][j] == 7) {
			// 	context.drawImage(listOfGhost[1].img, center.x-half_square, center.y-half_square ,square,square);
	
			// } else if (board[i][j] == 8) {
			// 	context.drawImage(listOfGhost[2].img, center.x-half_square, center.y-half_square ,square,square);
				
			// } else if (board[i][j] == 9) {
			// 	context.drawImage(listOfGhost[3].img, center.x-half_square, center.y-half_square ,square,square);
			// }
