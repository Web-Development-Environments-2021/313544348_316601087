...
var upArrow = 38;
var downArrow = 40;
var ghostPositions = [[0,0], [0,9], [9,0], [9,9]];
var ghosts_amount=1;
...
function Start() {
	...
	strawInterval = setInterval(updateStrawPosition, 600);
...
function updateStrawPosition(){
		...
		if(board[focreStrawberry.col][focreStrawberry.row] == 2){
			score += 50;
			window.clearInterval(strawInterval);
			focreStrawberry.prev = 0;
			board[focreStrawberry.col][focreStrawberry.row] = 0; //pacman
		}
	...
	function getRandomDirS(straw){
		...
		else if(ghostRow+1 < 10 && !(notValidMove.includes(board[ghostCol][ghostRow+1]))){
				move.push("down")
			}
		...
		else if(ghostCol+1 < 10 && !(notValidMove.includes(board[ghostCol+1][ghostRow]))){
				move.push("right");
			}
...
function updateGhostPosition() {
	for(var m=0 ; m< ghosts_amount ; m++){
		if(listOfGhost[m].prev == 2){ //pacman
...
function getRandomDir(ghost){
	...
	if (ghostRow > pacmanRow) {
		...
		if(ghostRow+1 < 10 && !(notValidMove.includes(board[ghostCol][ghostRow+1]))){
			move.push("down")
		}
	}
	if (ghostCol > pacmanCol) {
		...
		if(ghostCol+1 < 10 && !(notValidMove.includes(board[ghostCol+1][ghostRow]))){
			move.push("right");
		}
	}
		 // get random index value
...
function show(target){
	...
	document.getElementById(target).style.display = 'footer';
