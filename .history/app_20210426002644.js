...
function Start() {
	...
	var food_remain_list = {"food_5_points_remain": food_5_points_remain, "food_15_points_remain": food_25_points_remain, "food_25_points_remain": food_25_points_remain};;
	var pacman_remain = 1;
	start_time = new Date();
	for (var i = 0; i < 10; i++) {
		board[i] = new Array();
			...
			if (
				...
				if (randomNum <= (1.0 * food_remain) / cnt) {
					if(food_5_points_remain > 0){
						food_remain_list.push("food_5_points_remain");
						// food_5_points_remain--;
						// board[i][j] = 5;
						// food_remain--;
					}
					if(food_15_points_remain > 0){
						food_remain_list.push("food_5_points_remain");
						// food_15_points_remain--;
						// board[i][j] = 15;
						// food_remain--;
					}
					if(food_25_points_remain > 0){
						food_remain_list.push("food_5_points_remain");
					...
					var key = Math.floor(Math.random() * Object.keys(food_remain_list).length);
					var myString = Object.keys(food_remain_list)[key];// the key
