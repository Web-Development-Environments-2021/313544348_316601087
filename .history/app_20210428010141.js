	...
	for (var i = 0; i < boardLength; i++) {
		for (var j = 0; j < boardLength; j++) {
			...
			if (board[i][j] == 2) { //pacman
				...
				{ //up
					context.beginPath();
					context.arc(center.x, center.y, 15, 1.70 * Math.PI , 1.30 * Math.PI); // half circle
					context.lineTo(center.x, center.y);
					context.fillStyle = pac_color; //color
					context.fill();
					context.beginPath();
					context.arc(center.x - 6, center.y , 5,2 * Math.PI, 0); // circle
				...
				{ //left
					context.beginPath();
					context.arc(center.x, center.y, 15, 1.2 * Math.PI , 0.88 * Math.PI); // half circle
					context.lineTo(center.x, center.y);
					context.fillStyle = pac_color; //color
					context.fill();
					context.beginPath();
					...
					context.beginPath();
					context.arc(center.x, center.y, 15, 0.2 * Math.PI, 1.88 * Math.PI); // half circle
					context.lineTo(center.x, center.y);
					context.fillStyle = pac_color; //color
					context.fill();
					context.beginPath();
				...
				context.beginPath();
				context.rect(center.x - 15, center.y - 30, 30, 30);
				context.fillStyle = "grey"; //color
				context.fill();
			//monsters -  add check pos for animation
			} else if (board[i][j] == 6) {
				context.drawImage(listOfGhost[0].img , center.x-15, center.y-30 ,30,30);
				
			} else if (board[i][j] == 7) {
				context.drawImage(listOfGhost[1].img, center.x-15, center.y-30 ,30,30);
	
			} else if (board[i][j] == 8) {
				context.drawImage(listOfGhost[2].img, center.x-15, center.y-30 ,30,30);
				
			} else if (board[i][j] == 9) {
				context.drawImage(listOfGhost[3].img, center.x-15, center.y-30 ,30,30);
			}
	        else if (board[i][j] == 50) {
				// context.shadowBlur = 5;
				// context.shadowColor = "white";
				context.drawImage(focreStrawberry.img, center.x-15, center.y-15 ,30,30);
			}
			else if (board[i][j] == 100) {
				context.drawImage(heart.img, center.x-15, center.y-15 ,30,30);
			}

			else if (board[i][j] == 200) {
				context.drawImage(candy.img, center.x-15, center.y-15 ,30,30);
			}
