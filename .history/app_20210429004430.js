...
function Start() {
	...
	foodToEat = food_remain + 3; //adding the heart, candy and strawberry
...
function UpdatePosition() {
	...
	for(var x=0 ; x< ghosts_amount ; x++){
		if(board[listOfGhost[x].col][listOfGhost[x].row] ==2 && (board[shape.i][shape.j] == 6 || board[shape.i][shape.j] == 7 || board[shape.i][shape.j] == 8  || board[shape.i][shape.j] == 9))
		{
			listOfGhost[x].prev = 0;
		}
		board[listOfGhost[x].col][listOfGhost[x].row] = listOfGhost[x].prev;
	}
		initialGhosts(board);
...
// 		if(listOfGhost[m].col == shape.i && listOfGhost[m].row == shape.j){	
// 			// listOfGhost[m].prev = 0;
// 			haveMet = true;
// 			break;
// 		}
