...
function Draw() {
	...
	for (var i = 0; i < 10; i++) {
		for (var j = 0; j < 10; j++) {
			...
			// }
			}if(i == pinki.i && j == pinki.j && ghosts_amount >= 1 ){//1234
				context.drawImage(pinki.img, center.x-30, center.y-30 ,60,60);
			}
			if(i == redi.i && j == redi.j && ghosts_amount >= 2){//234
				context.drawImage(redi.img, center.x-30, center.y-30 ,60,60);
			}
			if(i == bluei.i && j == bluei.j && ghosts_amount >= 3){//34
				context.drawImage(bluei.img, center.x-30, center.y-30 ,60,60);
			}
			if(i == yellowi.i && j == yellowi.j && ghosts_amount >= 4){//4
				context.drawImage(yellowi.img, center.x-30, center.y-30 ,60,60);
			}
		}
	}
...
function updateGhostPosition() {
	for(var m=0 ; m< ghosts_amount ; m++){
		...
		if(dir == 'up'){
			...
			listOfGhost[m].i = listOfGhost[m].j-1;
		}
		else if(dir == 'right'){
			listOfGhost[m].i = listOfGhost[m].j+1;
		}
...
function UpdatePosition() {
	...
	if ((pinki.i == shape.i && pinki.j == shape.j) || (redi.i == shape.i && redi.j == shape.j) || (bluei.i == shape.i && bluei.j == shape.j)  || (yellowi.i == shape.i && yellowi.j == shape.j)) 
	{
		score = score - 10;
		if (lifes > 1)
		{
