...
function Start() {
	...
	ghostInterval = setInterval(updateGhostPosition, 159);
}
	...
	for (var i = 0; i < 10; i++) {
		for (var j = 0; j < 10; j++) {
			...
			// }
				...
				path = []; 
				context.beginPath();
				context.fillStyle = bodyCol;
				context.arc(x , y, width, Math.PI, 2* Math.PI);
				context.lineTo(x + width, y + height);
				context.arc(x + width / 2, y + height, width * 0.5, 0, Math.PI);
				context.arc(x + width / 2 - width*0.5 , y + height, width * 0.5, 0, Math.PI);
				context.arc(x + width / 2 - width , y + height, width * 0.5, 0, Math.PI);
				context.closePath();
				context.fill();
				context.beginPath();
				context.arc(x + 5, y, 5, 0, 2 * Math.PI); // circle
...
function getRandomDir(ghost){
	...
	var ghostCol = ghost.i;
	var ghostRow = ghost.j;
	// pacmanX = shape.i;
	// pacmanY = shape.j;
	// ghostX = ghost.i;
	// ghostY = ghost.j;
	...
	if (ghostRow > pacmanRow) {
		if(ghostRow-1 >= 0 && board[ghostCol][ghostRow-1] != 4){
			move.push("up");
		}
	} else if (ghostRow < pacmanRow) {
		if(ghostRow+1 < 10 && board[ghostCol][ghostRow+1] != 4){
			move.push("down")
		}
	}
	if (ghostCol > pacmanCol) {
		if(ghostCol-1 >= 0 && board[ghostCol-1][ghostRow] != 4){
			move.push("left");
		}
	} else if (ghostCol < pacmanCol) {
		if(ghostCol+1 < 10 && board[ghostCol+1][ghostRow] != 4){
			move.push("right");
		}
	}
