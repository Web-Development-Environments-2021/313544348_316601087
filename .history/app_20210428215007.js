...
function checkForMeeting(){
	...
	{
		if(listOfGhost[m].prev ==2 || (shape.i == listOfGhost[m].row && shape.j == listOfGhost[m].col)){ 
			haveMet = true;
		}
	}
	if(haveMet == true){
	...
	//straw 
	else if(board[shape.i][shape.j] == 50 || focreStrawberry.prev == 2){
		...
		window.clearInterval(strawInterval);
		focreStrawberry.prev = 0;
		board[focreStrawberry.col][focreStrawberry.row] = 2;
	}
}
